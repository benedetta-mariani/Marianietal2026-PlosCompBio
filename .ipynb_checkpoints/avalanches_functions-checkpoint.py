...
def plot_crackling_noise(ax, args, cpoints, cpred,cfit,
    ...
    #ax.scatter(durations[(durations<xmax) & (durations>xmin)][::1000],
    #           sizes[(durations<xmax) & (durations>xmin)][::1000],
    #           color = cpoints, alpha = 0.01, s = 10)
    ...
    ax.scatter(a[(a<xmax) & (a>xmin)], b[(a<xmax) & (a>xmin)],
               color = cfit, s = s, alpha = alphamarker)
    if prediz:
        ax.plot(x, (10**inter)*x**pred, cpred,
                label = r'$\delta^\mathrm{'+label+'}_\mathrm{pred} =%2.2f \pm %2.2f$'%(round(pred,2),round(errpred+0.01,2)),
                lw = lwline, ls = '--', zorder = 10000)
    ax.plot(x, (10**inter)*x**fit, color = cfit,
            label = r'$\delta^\mathrm{'+label+'}_\mathrm{fit} \,\,=%2.2f \pm %2.2f $'%(round(fit,2),round(errfit+0.01,2)),
            lw = lwline, ls = '--', zorder = 10000)

    ax.fill_between(x,(10**inter)*x**(fit + errfit*-3),
                    (10**inter)*x**(fit+ errfit*3),color = cfit, lw = 0, alpha = 0.1)
    if prediz:
        ax.fill_between(x,(10**inter)*x**(pred+errpred*-3),
                        (10**inter)*x**(pred+errpred*+3),color = cpred, lw = 0,alpha = 0.1)
        
    ax.set_yscale('log')
    ax.set_xscale('log')
    #ax.legend(handletextpad=0.4, handlelength = 0.5, scatteryoffsets = [0.5],
    #          fontsize = 20, loc = 'upper left')#20
